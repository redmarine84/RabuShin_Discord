...
Public NotInheritable Class DiscordSpellService
    ...
    Private Shared Function LoadSpells() As List(Of SrdSpellReference)
        Dim candidates As String() = {
            Path.Combine(AppContext.BaseDirectory, "Data", "srd_spells_5_2_1.json"),
            Path.Combine(AppContext.BaseDirectory, "srd_spells_5_2_1.json")
        }
        Dim path = candidates.FirstOrDefault(Function(p) File.Exists(p))
        If String.IsNullOrWhiteSpace(path) Then Return New List(Of SrdSpellReference)()
        Dim options As New JsonSerializerOptions With {.PropertyNameCaseInsensitive = True}
        Return JsonSerializer.Deserialize(Of List(Of SrdSpellReference))(File.ReadAllText(path), options)
    End Function
