...
function renderEconomyServices(services) {
  ...
  const commissions=(services.commissions||[]).map(item=>`<article class="economy-service-card economy-commission-card"><div><small>COMMISSION \u2022 ~20 MIN</small><h4>${escapeHtml(item.itemName)}</h4><p>${escapeHtml(item.description||'Commission this item even when ordinary stock is unavailable.')}</p></div><div class="economy-commission-action"><b>Commission \u2022 ${formatShopGp(item.commissionPriceGp)}</b><button class="button primary economy-service-button" data-service-type="commission" data-stock-id="${escapeHtml(item.stockId)}">Commission</button></div></article>`).join(''); // BLACKSMITH COMMISSION LAYOUT HOTFIX V2
